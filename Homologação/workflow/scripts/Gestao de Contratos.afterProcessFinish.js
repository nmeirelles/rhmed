function afterProcessFinish(processId){
	...
	for(var i = 0; i < dsSLACF.rowsCount; i++){
		...
		hAPI.setCardValue("dataDisponibilidadeS1", dataDisponibilidadeCF);
		hAPI.setCardValue("dataAssumiuS1", dataAssumiuCF);
		hAPI.setCardValue("dataFinalizouS1", dataFinalizouCF);
		hAPI.setCardValue("usuarioS1", usuarioCF);
	}
	// Get SLA - Contas a Pagar
	var numeroSolicitacaoCP = processId;
	var numeroAtividadeCP = 110;
	var numeroProximaAtividadeCP = 113;
	var nomeGrupoCP = "Pool:Group:Aprovação Suprimentos";
	...
	for(var i = 0; i < dsSLACP.rowsCount; i++){
		...
		hAPI.setCardValue("dataDisponibilidadeS2", dataDisponibilidadeCP);
		hAPI.setCardValue("dataAssumiuS2", dataAssumiuCP);
		hAPI.setCardValue("dataFinalizouS2", dataFinalizouCP);
		hAPI.setCardValue("usuarioS2", usuarioCP);
	}
