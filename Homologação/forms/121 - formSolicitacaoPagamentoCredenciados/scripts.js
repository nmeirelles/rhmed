						...
						FLUIGC.modal({
							...
							if(error){
								...
								$("#tableModalFornecedor").on('click', 'tr', function(){
									...
									validaDuplicidadeNF(); // VALIDA DUPLICIDADE
								});
							}
						});
					}
...
function validaDuplicidadeNF(){ //Alerta o usuário caso possua NF duplicada para o CNPJ.
	console.log("RUN validaDuplicidadeNF()");
	
	cnpj = $('#cnpj').val();
	numeroNF = $('#inputNrNotaFiscal').val();
	...
	if(cnpj != "" && numeroNF != "" && cnpj != null && numeroNF != null){
		...
		for(var c = 0; c < dsCount.length ; c++){
			if (dsCount[c].values != undefined){
				if(dsCount[c].values.length > 0){
					for(var i = 0; i < dsCount[c].values.length ; i++){
						//captura o numero do processo aberto
						var numFluxo = dsCount[c].values[i]["numeroFluxo"];
						var processoAtual = $('#numeroFluxo').val();
						...
						if(datasetProcess.values.length > 0){
							...
							var status = datasetProcess.values[0]["status"];
						}
					}
					
					if(dsCount[c].values.length > 0 && status != 1 && numFluxo != processoAtual){
						exibirMensagem("","Nota Fiscal / RPA já lançada para este CNPJ na solicitação <b>"+numFluxo+"</b>.","danger");
					}
				}
			}
		}
