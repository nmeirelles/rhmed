function validateForm(form){
	...
	var proximaAtividade = getValue("WKNextState");
		...
		if (atividadeAtual == 0 || atividadeAtual == 2 || atividadeAtual == 26){
			...
			let valorTotal = form.getValue("valorRequisicao");
			let nrBanco = form.getValue("banco");
			let nrAgencia = form.getValue("agencia");
			let nrConta = form.getValue("conta");

			let tipoRequisicao = form.getValue("tipoPagamento");
			let obsSolicitacao = form.getValue("obsDadosSolicitacao");
			if (campoVazio(form, "colabForn")){
				msgErro += "<li>Colaborador ou Fundo Fixo</li>";
				...
				msgErro += "<li>Colaborador</li>";
			}if (campoVazio(form, "descricao")){
				msgErro += "<li>Motivo da Prestação de Conta";
			}if (campoVazio(form, "existeAdiantamento")){
				msgErro += "<li>Condição da Prestação de Contas";
			...
			if(indexesItems.length < 1){
				msgErro += "<li>Necessário ao menos incluir uma linha de Item.</li>";
			}

			var indexesItems = form.getChildrenIndexes("tabelaItens");
			if(indexesItems.length >= 1){
				for (var i = 0; i < indexesItems.length; i++) {
					if (campoVazio(form, "zoomItens___"+indexesItems[i])){
						msgErro += "<li>Campo Descrição do Item | Item: "+indexesItems[i]+".</li>";
					}
					if (campoVazio(form, "valorTotalItem___"+indexesItems[i])){
						msgErro += "<li>Campo Valor Total | Item: "+indexesItems[i]+".</li>";
					}
					if (campoVazio(form, "contaCredito___"+indexesItems[i])){
						msgErro += "<li>Conta Crédito | Item: "+indexesItems[i]+".</li>";
					}
					if (campoVazio(form, "contaDebito___"+indexesItems[i])){
						msgErro += "<li>Conta Débito | Item: "+indexesItems[i]+".</li>";
					}
				}
			}
		}
...
function consultaSP(numeroFluig, filial){
	var c1 = DatasetFactory.createConstraint("tipoSP", "SPC", "SPC", ConstraintType.MUST);
