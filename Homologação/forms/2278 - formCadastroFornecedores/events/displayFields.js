function displayFields(form, customHTML){
	...
	if (atividade != 0 && atividade != 4){
		var cnpj = form.getValue("inputCNPJ");
		form.setValue("hiddenInputCNPJ", cnpj);
		form.setVisibleById('divDadosCadastrais', true);
	}

	if(atividade != 0 && atividade != 4 && atividade != 11){
		form.setVisibleById('divConsultaFornecedores', false);		
		var formaPagamento = form.getValue("formaPagamento");
		if(formaPagamento == "Credito em Conta"){
			form.setVisibleById('divNumeroBoleto', false);
			form.setVisibleById('divNumeroConta', true);
		}
		if(formaPagamento == "Boleto"){
			form.setVisibleById('divNumeroBoleto', true);
			form.setVisibleById('divNumeroConta', false);
		}
		form.setVisibleById('documentoAnexo', false);
		form.setVisibleById('divBtnBuscaCEP', false);
	}else{
		form.setVisibleById('documentoAnexo', true);
		form.setVisibleById('divBtnBuscaCEP', true);
	}
	...
	if(atividade == 107 || atividade == 14){
		form.setVisibleById('divCelulaCadastro', true);
	}else{
		form.setVisibleById('divCelulaCadastro', false);
	}
	...
	if (atividade == 5 || atividade == 99 || atividade == 107 || atividade == 96 || atividade == 14){
