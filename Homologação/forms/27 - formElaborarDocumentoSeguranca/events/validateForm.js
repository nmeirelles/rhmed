function validateForm(form){
	...
	if ((atividadeAtual == 38) && (profissionalVist == "cred")){
		if (campoVazio(form, "razaoSocialCred")){
			msgErro += "<li>Credenciado</li>";
		}
		
		if (campoVazio(form, "cnpjCred")){
			msgErro += "<li>CNPJ do Credenciado</li>";
		}
	}
...
function validaPedido(form){
	...
	if (indexes.length < 1){
		...
		for (var i = 0; i < indexes.length; i++) {
			if (form.getValue("zoomOC___" + indexes[i]) == null || form.getValue("zoomOC___" + indexes[i]) == ""){
				throw ("Campo <b>Pedido</b> Obrigatório \n\r");
			}
			if (form.getValue("np___" + indexes[i]) == null || form.getValue("np___" + indexes[i]) == ""){
				throw ("Campo <b>Número do Pedido</b> Obrigatório \n\r");
			}
			if (form.getValue("dataEmissaoOC___" + indexes[i]) == null || form.getValue("dataEmissaoOC___" + indexes[i]) == ""){
				throw ("Campo <b>Data de Emissão</b> Obrigatório \n\r");
			}
			if (form.getValue("codCondicaoPgto___" + indexes[i]) == null || form.getValue("codCondicaoPgto___" + indexes[i]) == ""){
				throw ("Campo <b>Condição de Pagamento</b> Obrigatório \n\r");
			}
			if (form.getValue("valorPedido___" + indexes[i]) == null || form.getValue("valorPedido___" + indexes[i]) == ""){
...
function campoVazio(form, fieldname){
	if ((form.getValue(fieldname) == null) || (form.getValue(fieldname) == undefined) || (form.getValue(fieldname).trim() == "") || (form.getValue(fieldname).trim() == "vazio")){
		return true;
	} // if
	return false;
} // campoVazio
