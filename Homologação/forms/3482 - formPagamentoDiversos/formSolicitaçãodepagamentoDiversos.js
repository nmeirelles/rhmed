$(document).ready(function() {
	...
	$("#aprovacaoCoordenador").on('change', (event) => {
		let formaPagamento = event.target[event.target.selectedIndex].text;
		$("#aprovacaoCelulaFiscal").val(formaPagamento);
	})
	...
	const trataCódigoBarras = (nomeCampo, codigo) => { // 34191090658629450012851552340005788610000014809
		...
		let validacao = validaCodigoBarras(retiraEspaco);
		if(validacao == true) $("#"+nomeCampo).val(retiraEspaco);
		else {
			$("#"+nomeCampo).focusout().val("");
			FLUIGC.toast({title: 'Erro: ', message: "Código de barras inválido!", type: 'warning'});
		}
	...
	if(nrAtividade != 0 && nrAtividade != 1 && nrAtividade != 3){
...
function setSelectedZoomItem(selectedItem) {
	...
	if(selectedItem.inputId == 'zoomCentroCusto'){
		...
		tabelaAprovadores(selectedItem.ccusto);
	}
...
function tabelaAprovadores(selectedItem){
	let centroCusto = selectedItem;
	let matriculaSolicitante = $("#solicitanteMatricula").val();
	let c1 = DatasetFactory.createConstraint("ccusto", centroCusto, centroCusto, ConstraintType.MUST);
	let constraintsCentroCusto = new Array(c1);
	DatasetFactory.getDataset("dsCadastroCentrodeCusto", null, constraintsCentroCusto, null,{
		success:(result)=>{
			clearTable("tableAprovadores");

			// Supervisor
			let indexS = wdkAddChild("tableAprovadores");
			$("#inputNivel___"+indexS).val("Supervisor");
			let matriculaSupervisor = String(result.values[0].supervisor);
			$("#inputMatricula___"+indexS).val(matriculaSupervisor);
			//let nomeSupervisor = matriculaAprovadores(matriculaSupervisor);
			//$("#inputResponsavel___"+indexS).val(nomeSupervisor);
			if(matriculaSupervisor == matriculaSolicitante) $("#inputStatus___"+indexS).val("true");
			else $("#inputStatus___"+indexS).val("false");
			
			//Coordenador
			let indexC = wdkAddChild("tableAprovadores");
			$("#inputNivel___"+indexC).val("Coordenador");
			let matriculaCoordenador = String(result.values[0].coordenador);
			$("#inputMatricula___"+indexC).val(matriculaCoordenador);
			//let nomeCoordenador = matriculaAprovadores(matriculaCoordenador);
			//$("#inputResponsavel___"+indexC).val(nomeCoordenador);
			if(matriculaCoordenador == matriculaSolicitante) $("#inputStatus___"+indexC).val("true");
			else $("#inputStatus___"+indexC).val("false");

			//Gerente
			let indexG = wdkAddChild("tableAprovadores");
			$("#inputNivel___"+indexG).val("Gerente");
			let matriculaGerente = String(result.values[0].gerente);
			$("#inputMatricula___"+indexG).val(matriculaGerente);
			//let nomeGerente = matriculaAprovadores(matriculaGerente);
			//$("#inputResponsavel___"+indexG).val(nomeGerente);
			if(matriculaGerente == matriculaSolicitante) $("#inputStatus___"+indexG).val("true");
			else $("#inputStatus___"+indexG).val("false");
	
			//Diretor
			let indexD = wdkAddChild("tableAprovadores");
			$("#inputNivel___"+indexD).val("Diretor");
			let matriculaDiretor = String(result.values[0].diretor);
			$("#inputMatricula___"+indexD).val(matriculaDiretor);
			//let nomeDiretor = matriculaAprovadores(matriculaDiretor);
			//$("#inputResponsavel___"+indexD).val(nomeDiretor);
			if(matriculaDiretor == matriculaSolicitante) $("#inputStatus___"+indexD).val("true");
			else $("#inputStatus___"+indexD).val("false");
		},error:(err)=>{
			console.log(err);
		}
	});
...
function matriculaAprovadores(matricula){
	let c1 = DatasetFactory.createConstraint("colleaguePK.colleagueId", matricula, matricula, ConstraintType.MUST);
	let constraintsColleague = new Array(c1);
	let datasetColleague = DatasetFactory.getDataset("colleague", null, constraintsColleague, null);
