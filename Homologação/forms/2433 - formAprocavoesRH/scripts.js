$(document).ready(function () {
	...
	//Adicionado Busca de fornecedores --> 07/10/2022 --> Inicio
	$("#inputFornecedor").change('change', () => {
		$("#inputCodFornecedor").val('');
	});
	...
	$("#inputFornecedor").on('keyup', (event) => {
		if (event.keyCode == 13) {
			const valorPesquisado = event.target.value;
			const tipoBusca = $("#selectTipoBusca").val();
			if (valorPesquisado == "" || tipoBusca == ""){
				FLUIGC.toast({ title: 'Atenção: ', message: 'Favor informar o tipo de busca e a pesquisa a ser realizada!', type: 'warning' });
			} else {
				buscaFornecedores(valorPesquisado, tipoBusca);
			}
		}
	});
	...
	//Adicionado Busca de fornecedores --> 07/10/2022 --> Fim
	...
	$("#dataPagamento").on('blur', () => {
		...
		if (dataPagamento != "") {
			let comparacao = compararDatas(dataAtual, dataPagamento);
			if (comparacao == false) {
