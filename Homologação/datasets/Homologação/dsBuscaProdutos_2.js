...
function callService(fields, constraints, sortFields) {
	...
	if (constraints != null) {
        for (var i = 0; i < constraints.length; i++) {
            if (constraints[i].fieldName == "codprod") codprod = constraints[i].initialValue;
			if (constraints[i].fieldName == "pesq") pesq = constraints[i].initialValue;
        }
	}

	var serviceData = data(codprod, pesq);
	...
	var response = service.rhmws004C(getParamValue(params.empresa, assigns.empresa), getParamValue(params.filial, assigns.filial), 
