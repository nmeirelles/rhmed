...
function callService(fields, constraints, sortFields) {
	...
	var response = authIwsDataServer.rhmws004I(getParamValue(params.empresa, assigns.empresa), getParamValue(params.filial, assigns.filial));
...
function processResult(result) {
	...
	result = result.getTFORMAPAG();

		dataset.addColumn("cODFORMA");
	dataset.addColumn("dESCFORMA");

	for (var i = 0; i < result.size(); i++) {
		dataset.addRow([result.get(i).getCODFORMA(), result.get(i).getDESCFORMA()]);
	}
