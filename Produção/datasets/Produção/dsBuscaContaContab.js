...
function callService(fields, constraints, sortFields) {
	...
	var response = authIwsDataServer.rhmws004F(getParamValue(params.empresa, assigns.empresa), getParamValue(params.filial, assigns.filial)
		);
...
function processResult(result) {
	...
	result = result.getTCONTA();

		dataset.addColumn("cODCONTA");
	dataset.addColumn("dESCCONTA");

	for (var i = 0; i < result.size(); i++) {
		dataset.addRow([result.get(i).getCODCONTA(), result.get(i).getDESCCONTA()]);
	}
