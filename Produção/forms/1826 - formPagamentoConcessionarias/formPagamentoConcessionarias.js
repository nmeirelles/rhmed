$(document).ready(function() {
	...
	$("#numeroNota").on('change', () => {
		let emissorNF = $("#inputCodEmissorNF").val();
		if(emissorNF != "" && emissorNF != null && emissorNF != undefined){
			urlRest2 = "https://rhmedconsultores119082.protheus.cloudtotvs.com.br:4050/rest/WSCONSULTA?FIL=0101&TABELA=SF1&FILTRO=F1_DOC$"+$("#numeroNota").val()+"@F1_SERIE$3@F1_FORNECE$"+emissorNF;
			callRest(urlRest2);
		}
	});
						...
						$("#tableModalFornecedor").on('click', 'tr', function(){
							...
							if(codForn != undefined){
								...
								if(idCampo == "inputEmissorNF" || idCampo == "consultaEmissorNF"){
									...
									let numeroNF = $("#numeroNota").val();
									if(numeroNF != "" && numeroNF != null && numeroNF != undefined){
										urlRest2 = "https://rhmedconsultores119082.protheus.cloudtotvs.com.br:4050/rest/WSCONSULTA?FIL=0101&TABELA=SF1&FILTRO=F1_DOC$"+numeroNF+"@F1_SERIE$3@F1_FORNECE$"+$("#inputCodEmissorNF").val();
										callRest(urlRest2);
									}
								}
								$('#modalFornecedores').modal('toggle');
...
function callRest(urlRest){
	...
	$.ajax(settings).success(function(data){
		console.log(data);
		if(data.erro != undefined && data.erro != "" && data.erro != null){
			FLUIGC.toast({title: 'Liberado: ', message: "Nota Fiscal ainda não existe no Protheus!" , type: 'success', timeout: 2000});
		}else{
			FLUIGC.toast({title: 'Bloqueado: ', message: "Nota Fiscal já existe no Protheus!" , type: 'danger'});
			$("#numeroNota").val("");
		}
	}).fail(function(data){
		console.log(data);
