...
$(document).ready(function() {
	...
	//Campo SelectProcedimento Baseado no Campo zoomProcedimento - #tabelaProcedimentos
	...
	$("#selectProcedimento").append(optionsProcedimento);
	$("#selectProcedimentoR").append(optionsProcedimento);
	...
	if(CURRENT_STATE == "4" || CURRENT_STATE == "27"){
		setTimeout(() => {
...
/********************ESCONDER BOTOES PAI FILHO**************************/
	...
	if(atividade == "54"){
		let tabelaCredenciado = $("[tablename='tabelaCredenciado'] tbody tr");
	...
	if(atividade == "78"){
		...
		tabelaCredenciado.each(function(index, element){
			let inputProcedimentoReagendado = tabelaCredenciado.eq(index).find("[id^='inputProcedimentoReagendado']").val().split("-")[0];
			if(inputProcedimentoReagendado == "nao"){
				$("#NfRpa___" + index).prop("readonly", false);
				$("#dataEnvioMalote___" + index).prop("readonly", false);
			}
		});
		let tabelaReagendamento = $("[tablename='tabelaReagendamento'] tbody tr");
		tabelaReagendamento.each(function(index, element){
...
function removedZoomItem(removedItem) {		
	...
	if (removedItem.inputId == "zoomPlataforma") {
		...
		window["nomeFilial"].clear();
		$('#codBuscaEmpresa').val('');
		$('#codNomeBuscaEmpresa').val('');
		$('#codNomeBuscaUnidade').val('');
		$('#vidasAtivas').val('');
	}
	
	if (removedItem.inputId == "nomeEmpresa") {
		window["zoomBuscaRegional"].clear();
		window["nomeFilial"].clear();
		$('#codBuscaEmpresa').val('');
		$('#codNomeBuscaEmpresa').val('');
	...
	if (removedItem.inputId == "zoomBuscaRegional") {
		window["nomeFilial"].clear();
		$('#codNomeBuscaUnidade').val('');
		$('#vidasAtivas').val('');
	}
	
	if (removedItem.inputId == "nomeFilial") {
		$('#codNomeBuscaUnidade').val('');
		$('#vidasAtivas').val('');
	}
...
function calcularTempo(event) {
	...
	if(id == "horaAtendEntrada" || id == "horaAtendSaida"){
		...
		if (validar.test(entradaTempo) && validar.test(saidaTempo)) {
			...
			$('#perAlocado___'+ index).val(diferenca);
...
function getSelectProcedimento(event){
	...
	if(inputId == "selectProcedimento"){
		...
		document.getElementById("procedSelecionado___"+index).value = selectProcedimento;
	...
	input.value = "";
	//document.getElementById("procedSelecionado___"+index).value = selectValue;
}
...
function habilitaDataContratoDomiciliar(){
	...
	const tabelaRow = $("[tablename='tabelaProcedimentos'] tbody tr");
...
function desabilitaPaiFilhoCredenciado(tabela){
	...
	if(tabela == "tabelaCredenciado"){
		const tabelaRow = $("[tablename='tabelaCredenciado'] tbody tr");
		tabelaRow.each(function(index, element) {
			if(index > 0){
				// campos
				tabelaRow.eq(index).find("[id^='cpfCnpjPrestador']").prop("readonly", true);
...
function alterarTipoCredenciado(event,id2 = "",index2 = ""){
	...
	if(id == "tipoContratacao"){
		let credenciado = document.getElementById("tipoContratacao___" + index).value;
		
		if (credenciado == "cred") {
			camposOnchangeCredenciado();
			
			document.getElementById("cpfCnpjPrestador___" + index).value = ("");
			//////////////////////////////////////////////////////////////////////
			document.getElementById("nomePrestador___" + index).value = ("");
			...
			document.getElementById("cpfCnpjPrestador___" + index).value = ("");
			//////////////////////////////////////////////////////////////////////
			document.getElementById("nomePrestador___" + index).value = ("N/D");
...
function camposOnchangeCredenciado(){
	...
	tabelaRow.each(function(index, element) {
		if(index > 0){
			let credenciado = tabelaRow.eq(index).find("[id^='tipoContratacao']").val();
			
			if (credenciado == "cred") {
				tabelaRow.eq(index).find("[id^='cpfCnpjPrestador']").prop("readonly", false);
...
function mesmoCredenciado(e){
	...
	if(index > 1){
		...
		if(reagendamento == "R"){
			...
			let tipoContratacao = document.getElementById("tipoContratacao___"+indexAnterior).value;
			let cpfCnpjPrestador = document.getElementById("cpfCnpjPrestador___"+indexAnterior).value;
			let nomePrestador = document.getElementById("nomePrestador___"+indexAnterior).value;
			...
			document.getElementById("tipoContratacao___"+index).value = tipoContratacao;
			document.getElementById("cpfCnpjPrestador___"+index).value = cpfCnpjPrestador;
			document.getElementById("nomePrestador___"+index).value = nomePrestador;
...
function desbloquearProcedimentoCredenciado(index){
	$("#selectProcedimento___" + index).removeClass("disabled");
	$("#tipoContratacao___" + index).removeClass("disabled");
	$("#cpfCnpjPrestador___" + index).prop("readonly", false);
	$("#nomePrestador___" + index).prop("readonly", false);
...
function reagendarProcedimento(event) {
	...
	if(reagendado == "nao"){
		...
		if(CURRENT_STATE == "37" || CURRENT_STATE == "48" || CURRENT_STATE == "50" || CURRENT_STATE == "69" || CURRENT_STATE == "71"){
			...
			if(id == "reagendar"){
				$("#selectProcedimento___" + index).addClass("disabled");
				$("#tipoContratacao___" + index).addClass("disabled");
				$("#cpfCnpjPrestador___" + index).prop("readonly", true);
				$("#nomePrestador___" + index).prop("readonly", true);
				...
				$("#procedSelecionadoR___"+newRow).val($("#procedSelecionado___"+index).val());
